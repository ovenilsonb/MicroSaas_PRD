    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
          isChemical ? 'bg-blue-50 text-[#202eac]' : 'bg-emerald-50 text-emerald-600'
        }`}>
      ...
      <h3 className="font-bold text-slate-800 text-sm mb-1 line-clamp-2">{ingredient.name}</h3>
      
      {ingredient.apelido && (
        <p className="text-xs text-slate-400 mb-2">{ingredient.apelido}</p>
      )}
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-3">
        ...
        <div className="text-right">
          ...
          <span className={`font-bold text-sm ${isLowStock ? 'text-amber-600' : 'text-slate-700'}`}>
